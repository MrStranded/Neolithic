...
public class RotatingCubeGame implements Updater {
	...
	public void init() {
		...
		triangle = new Mesh();
		triangle.getMeshData().setVertexBuffer(BufferUtils.createVector3Buffer(3));

		final byte[] indices = {0, 1, 2};
		final ByteBuffer bbuf = BufferUtils.createByteBuffer(indices.length);
		bbuf.put(indices);
		bbuf.rewind();
		triangle.getMeshData().setIndexBuffer(bbuf);

		triangle.getMeshData().setIndexMode(IndexMode.Triangles);

		triangle.getMeshData().getVertexBuffer().clear();

		triangle.getMeshData().getVertexBuffer().put(-5); // first
		triangle.getMeshData().getVertexBuffer().put(0);
		triangle.getMeshData().getVertexBuffer().put(0);
		triangle.getMeshData().getVertexBuffer().put(5); // second
		triangle.getMeshData().getVertexBuffer().put(0);
		triangle.getMeshData().getVertexBuffer().put(0);
		triangle.getMeshData().getVertexBuffer().put(0); // third
		triangle.getMeshData().getVertexBuffer().put(5);
		triangle.getMeshData().getVertexBuffer().put(0);
		triangle.setDefaultColor(new ColorRGBA(0,1f,0,1f));

		scene.getRoot().attachChild(triangle);
