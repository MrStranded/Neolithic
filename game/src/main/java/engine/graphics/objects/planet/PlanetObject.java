...
public class PlanetObject extends MoveableObject {
	...
	public void render(ShaderProgram shaderProgram, boolean sendMaterial, Matrix4 viewMatrix) {
		Matrix4 viewWorldMatrix = viewMatrix.times(getWorldMatrix());

		if (faceParts != null) {
			for (int i=0; i<20; i++) {
				if (faceParts[i] != null) {
					faceParts[i].render(shaderProgram, sendMaterial, viewWorldMatrix, depth);
				}
			}
		}
	}
